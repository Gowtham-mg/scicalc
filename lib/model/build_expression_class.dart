...
class BuildExpressionClass {
  buildExpressionComplex() {
    ...
    builder.group()
      ...
      ..left(
          string(kLogSign).trim(), (a, op, b) => a * (math.log(b) / math.ln10))
      ...
      ..left(char('C').trim(),
          (a, op, b) => factorial(a) / ((factorial(a - b)) * factorial(b)))
      ...
      ..prefix(
          string(kSinh).trim(), (op, l) => ((math.exp(l)) - math.exp(-l)) / 2)
      ..prefix(
          string(kCosh).trim(), (op, l) => (math.exp(l) + math.exp(-l)) / 2)
      ..prefix(
          string(kTanh).trim(),
          (op, l) =>
              (math.exp(l) - math.exp(-l)) / (math.exp(l) + math.exp(-l)))
